...
test('create and delete an equipment', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Agregar Equipo' }).click();
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro 16' }).click();
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro 16' }).fill('pc de mesa prueba');
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro M3 Max' }).click();
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro M3 Max' }).fill('32');
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro M3 Max' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro M3 Max' }).fill('32GB');
  await page.getByRole('textbox', { name: 'Ej: MacBook Pro M3 Max' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Describe las características' }).click();
  await page.getByRole('textbox', { name: 'Describe las características' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Describe las características' }).fill('R');
  await page.getByRole('textbox', { name: 'Describe las características' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Describe las características' }).fill('');
  await page.getByRole('textbox', { name: 'Describe las características' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Describe las características' }).fill('I');
  await page.getByRole('textbox', { name: 'Describe las características' }).press('CapsLock');
  await page.getByRole('textbox', { name: 'Describe las características' }).fill('Intel 5i');
  await page.getByRole('combobox').filter({ hasText: 'Seleccionar categoría' }).click();
  await page.getByLabel('🖥️ Computador de escritorio').getByText('🖥️ Computador de escritorio').click();
  await page.getByRole('button', { name: 'Crear Equipo' }).click();
  await page.locator('div:nth-child(9) > div:nth-child(3) > button').first().click();
